...
class ContinuousCartPoleEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        if render_mode in self.__class__.metadata["render.modes"] + [None]:
            ...
            raise (
                " render_mode <{render_mode}> not in ",
                self.__class__.metadata["render.modes"] + [None],
            )
...
class Buffer:
    def __init__(
        ...
        self.next_state_buffer = np.zeros(
            (self.buffer_capacity, num_states), np.float32
        )
    ...
    def sample(self) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        next_state_batch = Tensor(
            self.next_state_buffer[batch_indices], requires_grad=False
        )
...
class DeepDeterministicPolicyGradient:
    ...
    def __init__(
        ...
        self.actor_optimizer = optim.AdamW(actor_params, lr_actor)
        self.critic_optimizer = optim.AdamW(critic_params, lr_critic)
...
if __name__ == "__main__":
    ...
    if spaces.Box != env.action_space.__class__:
        raise Exception("The environment need to have a continuous action space")
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % print_interval == 0:
            ...
            print(
                f"Episode {episode}/{num_episodes} - average reward: {cumulative_reward/print_interval} of the last {print_interval} episode"
            )
            cumulative_reward = 0.0
            # # save best model
